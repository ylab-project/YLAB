...
for ib_ = 1:nb
  ...
  if stype_ == PRM.BWFS || stype_ == PRM.BHSS ...
      || stype_ == PRM.BHSR
...
return
  ...
  function add_row(inb)
    ...
    for ij = 1:npair
      ...
      if ij == 1
        ...
        bpbody(irow,3:4) = ...
          nominal_brace.coord_name(inb,1:2);
  ...
  function write_left_columns(idm_, idsb_)
    ...
    if secb.type(idsb_) == PRM.TB
      bpbody{irow,11} = '引張';
      bpbody{irow,12} = '-1.0';
    elseif is_tension(idm_)
      bpbody{irow,11} = '引張';
      bpbody{irow,12} = ...
        sprintf('%.1f', lam_e(idm_));
    else
      bpbody{irow,11} = '引圧';
      bpbody{irow,12} = ...
        sprintf('%.1f', lam_e(idm_));
