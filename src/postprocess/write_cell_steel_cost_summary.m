function [head, body] = ...
  write_cell_steel_cost_summary(com, options, cost, secdim)
...
if isempty(all_is)
  ...
  [grp, ia, ~] = unique( ...
    [all_sl, all_sn, all_im], 'rows');
  nrows = size(grp, 1);
  rep_is = all_is(ia);

  % 部位別重量の集計
  wt = zeros(nrows, npart);
  wt = accum_grp(wt, grp, secdim, ...
    cost.column, mc, IPCOL);
  wt = accum_grp(wt, grp, secdim, ...
    cost.girder, mg, IPGIR);
  wt = accum_grp(wt, grp, secdim, ...
    cost.brace, mb, IPBRC);
  if IPHBR > 0
    wt = accum_grp(wt, grp, secdim, ...
      cost.hbrace, mh, IPHBR);
  end
...
function [wt, grp, rep_is] = sort_by_type( ...
  wt, grp, rep_is, nrows, secmgr)
...
function blk = build_steel_weight_block( ...
  grp, rep_is, wt, nrows, npart, ...
  NFIX, NCOL, stype, secdim, secmgr, material)
...
%     NFIX, NCOL, stype, secdim, secmgr, material)
  ...
  for r = 1:nrows
    ...
    blk{r, 2} = tname;
    blk{r, 3} = format_steel_cost_dim( ...
      stype_, secdim(is, :), sym);
    blk{r, 5} = mname;
    for ip = 1:npart
      if wt(r, ip) > 0
        blk{r, NFIX + ip} = ...
...
function [blk, sub2] = build_extra_weight_block( ...
  grp, wt, nrows, npart, IPTOT, ...
  NFIX, NCOL, pfac, material)
...
function blk = build_total_block( ...
  sub1, sub2, npart, NFIX, NCOL)
