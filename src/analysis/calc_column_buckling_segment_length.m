...
for inc = 1:nnmc
  ...
  ic_bot = imcs(1);
  nd_bot = cn1(ic_bot);
  d_bot = find_rc_beam_depth(nd_bot, gn1, gn2, ...
    stype_gir, Hg_gir);
  if d_bot > 0
    lmc_bk(ic_bot) = lmc_bk(ic_bot) - d_bot / 2;
  end

  % 上端: 最上セグメントの上端節点
  ic_top = imcs(nsub);
  nd_top = cn2(ic_top);
  d_top = find_rc_beam_depth(nd_top, gn1, gn2, ...
    stype_gir, Hg_gir);
  if d_top > 0
    lmc_bk(ic_top) = lmc_bk(ic_top) - d_top / 2;
  end
end
...
function D = find_rc_beam_depth(nd, gn1, gn2, ...
  stype_gir, Hg_gir)
%find_rc_beam_depth - 節点��接続するRC梁せいを取得
%
%   D = find_rc_beam_depth(nd, gn1, gn2,
%     stype_gir, Hg_gir) は、
%   指定節点に接続するRC梁のうち最��のせいDを返���。
%   RC梁がなければ0を返す。
%
%   入力引数:
%     nd        - 対象節点番号
