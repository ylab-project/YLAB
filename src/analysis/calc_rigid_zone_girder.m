function lrgirder = calc_rigid_zone_girder(mgstype, idmg2sfl, ...
  idmg2sfr, idscb2s, cbsDf, sdimgm, secdim, stype, gdir, ...
  cxl, idmg2n, idsup2n)
%calc_rigid_zone_girder - 梁の剛域長を計算
%
%   lrgirder = calc_rigid_zone_girder(mgstype, idmg2sfl, idmg2sfr, ...
%     idscb2s, cbsDf, sdimgm, secdim, stype, gdir, cxl, idmg2n, ...
%     idsup2n) は、SS7仕様書3.3.1に基づいて梁の剛域長を計算する。
%   RC梁は最小柱面を梁材軸方向へ換算してからα×梁せいを引き、
%   S梁は既存どおり最大柱面/2で計算する。
  ...
  for ilr = 1:2
    ...
    if min_col_size < inf  % 柱が存在する場合
      if mgstype(ig) == PRM.RCRS
        % RC梁は選択済み柱面を梁材軸方向へ換算してからαDを引く。
        rc_face_dimension(ig, ilr) = min_col_size;
      else  % WFS (S梁)
