...
%% マトリクス解析
[msprop, dvec, dnode, felement, ignored_moment, stn, stcn, ...
  Mc, C, vix, viy, rvec, rs, dfn, rvec0, rs0, rs_analysis0, ...
  Mc0, dfn0, state, sw, lf, lr, lmem, lnm, lb, Iy, Iz, gphiI, ...
  gphiAs, gphiAn, cphiI, cbs, baseline, node, story, floor, Cn, ...
...
if do_stress_eval
  ...
  [stress_out{:}] = eval_nominal_allowable_stress_ratio(msdim, stn, A, ...
    Iy, Iz, Zyc, C, mtype, mstype, isxdir_member, isydir_member, ...
    wgx, wgy, Em, Fm, idm2n, lb, lm, lm_bk_x, lm_bk_y, lnm, ...
    mejoint, nominal, isgmirrored, idmg2mng, idmc2mnc, options, ...
    beta, lcdir, idmc2st, com.column_bracing, Cn, nomgc, ...
    com.column_buckling_K);
...
result.columnFcApplicable = stress_result.column_fc_applicable;
...
result.cgsr = cgsr;
result.sw = sw;
result.ignored_moment = ignored_moment;
sw_f_lc = zeros(com.nnode,6,com.nlc);
sw_f_lc(:,:,1) = sw.f;
faddnode_report = com.faddnode - com.faddnode_report_excl;
result.nodal_equiv_load = com.fnode + faddnode_report ...
  - felement - sw_f_lc - ignored_moment.node;
result.lb = lb;
result.lf = lf;
result.lr = lr;
result.lm = lm;
...
result.rigid_zone_overflow = find_column_base_rigid_zone_overflow( ...
  lr.columnx, lr.columny, lr.columnn, lm(com.member.column.idme), ...
  com.member.column);
...
% 節点重量は全荷重と自重が確定した分析境界で一元化する
result.nodal_weight = calc_nodal_weight(com, sw, cxl, cyl, lm, idm2n);
result.state = state;
