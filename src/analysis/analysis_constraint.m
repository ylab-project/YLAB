...
%% マトリクス解析
[msprop, secdim, dvec, dnode, felement, stn, stcn, Mc, C, ...
  vix, viy, rvec, rs, dfn, rvec0, rs0, Mc0, dfn0, state, ...
  sw, lf, lr, lm, lm_weight, lnm, lb, Iy, Iz, gphiI, ...
  gphiN, cphiI, cbs, baseline, node, story, floor, Cn, ...
...
if coptions.consider_stress_ratio
  ...
  % ブレース・柱の座屈長さ用部材長を算出
  stype_sec_ = com.section.property.type;
  idsecg_ = com.section.girder.idsec;
  lm_bk = lm;
  lm_bk(mtype==PRM.BRACE) = ...
    calc_brace_buckling_length(com.member.brace, ...
    com.member.girder, node, stype_sec_, idsecg_, secdim);
  if options.column_member_length_type == 1
    % コンクリートとの重複を除く（RC梁D/2控除）
    lm_bk(mtype==PRM.COLUMN) = ...
      calc_column_buckling_segment_length( ...
      com.member.column, com.member.girder, ...
      com.nominal.column, stype_sec_, idsecg_, ...
      secdim, lm(mtype==PRM.COLUMN));
  end
...
if ~isempty(bkinfo)
  result.lbmax_x = bkinfo.lbcnmaxX;
  result.lbmax_y = bkinfo.lbcnmaxY;
else
  result.lbmax_x = [];
  result.lbmax_y = [];
end
