...
%     nominal_column, js, je, is_girder, onfg_col,
...
lbcn_ = calc_with_bracing(lmc, nnmc, idnmc2mc, js, je, ...
...
%--------------------------------------------------------------
function lbcn_ = calc_with_bracing(lmc, nnmc, idnmc2mc, ...
  js, je, is_girder, onfg_col, idmc2m)
%calc_with_bracing - 横補剛点を考慮した補剛間隔計算
%
%   lbcn_ = calc_with_bracing(lmc, nnmc,
%     idnmc2mc, js, je, is_girder, onfg_col,
%     idmc2m) は、セグメント境界の横補剛点を判定し、
%   非補剛境界ではセグメントの lmc を合算する。
%
%   入力引数:
%     lmc        - セグメント芯間距離 [nmc×1]
%     nnmc       - 名目柱数
...
for inmc = 1:nnmc
  ...
  if ncol == 1
    lbcn_(inmc,:) = lmc(imcs(1));
    continue
  end

  % セグメントを横補剛区間に結合
  spans = merge_spans(lmc, imcs, ncol, js, je, is_girder, onfg_col, ...
    idmc2m);
...
%--------------------------------------------------------------
function spans = merge_spans(lmc, imcs, ncol, js, je, ...
  is_girder, onfg_col, idmc2m)
%merge_spans - セグメントを横補剛区間に結合する
%
%   spans = merge_spans(lmc, imcs, ncol, js, je,
%     is_girder, onfg_col, idmc2m) は、
%   セグメント境界の横補剛点を判定し、非補剛境界
%   ではセグメントのlmcを合算した区間長を返す。
%
%   入力引数:
%     lmc        - セグメント芯間距離 [nmc×1]
%     imcs       - 名目柱内のセグメント番号列
