...
%   出力引数:
%     dfn0    - 直接入力値を反映した設計応力
%     nominal - 名目梁端側1/4位置の曲げモーメントと入力有無

nnm = size(dfn0, 1);
nlc = size(dfn0, 3);
nominal.M = nan(nnm, 2, nlc);
nominal.has_M = false(nnm, 2, nlc);
% 直接入力が1件も無いモデルでは名目梁の走査を行わない
if ~any(element.has_M, 'all') && ~any(element.has_Q, 'all')
  return
end
for ing = 1:size(nominal_girder.idmeg, 1)
  ...
  for ilc = 1:nlc
    for iside = 1:2
      ...
      if element.has_M(im, iside, ilc)
        nominal.M(inm, iside, ilc) = element.M(im, sample_index, ilc);
        nominal.has_M(inm, iside, ilc) = true;
