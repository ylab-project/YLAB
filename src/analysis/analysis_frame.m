...
slab.deck_height = com.member.girder.deck_height;
slab.deck_height_lower = com.member.girder.deck_height_lower;
story = com.story;
...
% 鉛直方向は柱・鉛直ブレースの取付位置だけを補剛点とする
nomgc.lb_vertical = calc_nominal_girder_vertical_interval( ...
  lm(mtype==PRM.GIRDER), idmeg, com.girder_bracing.vertical, ...
  nomgc.xc_design);
...
[lm_column_weight, lm_column_lower] = calc_column_weight_length( ...
...
[lm_girder_weight, weight_deduct] = ...
...
lm_lower_extension = zeros(size(lm));
lm_lower_extension(mtype==PRM.COLUMN) = lm_column_lower;
...
if options.consider_self_weight && options.consider_finishing_material
  sw = comp_self_weight(A, Asy, Iy, Em, Gm, lm_weight, ...
    lm_lower_extension, lm, lrxm, member_property, msdim, slab, ...
    cxl, cyl, nnode, mejoint, weight_deduct, options, member_column, ...
    brace_unit_weight, Df_foundation, mglevel, member_girder.isfg, ...
...
[dfn0, nominal_position0] = apply_element_load_position_stress( ...
  dfn0, element_position0, df0, nominal_girder, idmg2m, lm, lf, ...
  lcdir, is_rc_girder_nm);
...
nominal_position = superpose_element_load_position_stress( ...
  nominal_position0, lcdir, is_rc_girder_nm, n_beam);
dfn = apply_element_load_position_shear(dfn, nominal_position.Q, ...
...
nomgc.position.M = nominal_position.M;
