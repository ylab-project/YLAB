...
% isthrough_node = false(nblx, nbly, nblz);
...
for i=1:n
  ...
  floor_name{i,2} = tochar(data{i,1});
  if ~ismissing(data{i,7})
    floor_name{i,2} = tochar(data{i,7});
  end
  xcoord_name{i,1} = tochar(data(i,2));
  xcoord_name{i,2} = tochar(data(i,2));
  ycoord_name{i,1} = tochar(data(i,3));
  ycoord_name{i,2} = tochar(data(i,3));
end
...
% for i=1:n
%   isthrough_node(idx(i),idy(i),idz(i)) = true;
% end

%% ダミー層で柱梁がとりつかない節点は削除
isdummy_node(:,:,story.isdummy) = true;
isdummy_node = isdummy_node&~isused_girder;
