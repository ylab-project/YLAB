...
% 共通配列
inisecg = com.section.initial.girder;
inisecc = com.section.initial.column;
inisecb = com.section.initial.brace;
secg = com.section.girder;
secc = com.section.column;
secb = com.section.brace;
secdim = com.secmgr.dimension;
idsg2s = secg.idsec;
idsc2s = secc.idsec;
idsb2s = secb.idsec;
...
for i=1:ng
  ...
  switch secg.type(idsg)
    case PRM.WFS
      ...
      if ch(1:2)=="H-"
        sss = sscanf(ch,'H-%fx%fx%fx%fx%f');
      elseif ch(1:2)=="SH"
        sss = sscanf(ch,'SH-%fx%fx%fx%fx%f');
      elseif ch(1:2)=="HY"
        sss = sscanf(ch,'HY-%fx%fx%fx%fx%f');
      elseif ch(1:2)=="BH"
        sss = sscanf(ch,'BH-%fx%fx%fx%fx%f');
      else
        ME = MException('YLAB:InvalidSectionType', ...
          '登録されていない鉄骨形状です');
        throw(ME);
      end
  ...
  switch secb.type(idsb)
    ...
    case PRM.HSR
      ...
      if contains(ch, '○-')
        % ○-を含む形式
        sss = sscanf(ch, '○-%fx%f');
        secdim(ids,1:2) = sss(1:2);  % D（外径）とt（板厚）
      else
        % 数値x数値の形式を抽出（フォールバック）
        sss = regexp(ch, '([0-9.]+)x([0-9.]+)', 'tokens');
        if ~isempty(sss)
          secdim(ids,1) = str2double(sss{1}{1});  % D（外径）
          secdim(ids,2) = str2double(sss{1}{2});  % t（板厚）
        end
      end
  end
end
