function [xlist, xup, xdw, idvartarget, idvlist] = ...
  enumerateNeighborTf(obj, xvar, idvar, options)
%enumerateNeighborTf フランジ厚tfの近傍断面を列挙
%   [xlist, xup, xdw, idvartarget, idvlist] = enumerateNeighborTf(obj,
%     xvar, idvar, options) は、指定変数のフランジ厚tfについて近傍断面を
%     列挙します。
%
...
if all(idwfs2x(:,PRM.SECDIM_WFS_TF) ~= idvar)
  idvartarget = [];
  return
end
...
% 近傍解集合
[tfup, id] = max(upsec(:,PRM.SECDIM_WFS_TF));
if (tfup ~= tf0)
  xup = xvar;
...
[tfdw, id] = min(dwsec(:,PRM.SECDIM_WFS_TF));
if (tfdw ~= tf0)
  xdw = xvar;
  if (size(idvartarget, 1) == 1)
